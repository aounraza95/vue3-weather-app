<template>
    <div class="flex flex-col flex-1 items-center">
        <!-- Banner -->
        <div v-if="router.query.preview" class="text-white p-4 bg-weather-secondary w-full text-center">
            <p>
                You are currently previewing this city this "+" icon to start tracking this city.
            </p>
        </div>
        <!-- Weather Overview -->
        <div class="flex flex-col items-center text-white py-12">
            <h1 class="text-4xl mb-2">{{ router.params.city }}</h1>
            <p class="text-sm0mb-12">
                {{
        ...
        <div class="max-w-screen-md w-full py-12">
            <div class="mx-8 text-white">
                ...
                <div class="flex items-center" v-for="day in weatherData.daily" :key="day.dt">
                    ...
                    <img class="w-[50px] object-cover" :src="`https://openweathermap.org/img/wn/${day.weather[0].icon}@2x.png`" alt="" />
...
const getWeatherData = async () => {
    try {
        const weatherData = await axios.get(`https://api.openweathermap.org/data/3.0/onecall?lat=${router.query.lat}&lon=${router.query.lng}&exclude={part}&appid=${openweatherAPIkey}&units=imperial`);

        // current date and time.
        const localOffset = new Date().getTimezoneOffset() * 60000;
        const utc = weatherData.data.current.dt * 1000 + localOffset;
        weatherData.data.currentTime = utc + 1000 * weatherData.data.timezone_offset;
        // cal hourly weather offset 1
